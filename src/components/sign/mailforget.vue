			...
			<div v-show="regfrom" class="login-form">
				<form @submit.prevent="regist" class="regist-form">
					<div class="signUser">
						...
						<input type="text"  placeholder="请输入你已经注册的邮箱号" v-model="regist.phone" />
					</div>
					<div class="verCode">
						<span class="codes">
							<input type="text"  placeholder="验证码" v-model="regist.verificat" />
						</span>
					...
					<div class="loginSub">
						<input type="submit"  name="" value="获取短信验证码" />
...
<script>
	...
	export default {
		...
		methods: {
			regist: function() {
				for(let i = 0; i < 1; i++) {
					this.$http.jsonp('http://51kufei.com/index.php?m=Home&c=Userapi&a=sendEmail&callback=userapi&key=QUgFYq4L5n89ZG2Ab&p=' + this.regist.phone + '&forget=1').then(function(res) {
						this.data = res.body;
			...
			verifi_sub: function() {
				for(let i = 0; i < 1; i++) {
					this.$http.jsonp('http://51kufei.com/index.php?m=Home&c=Userapi&a=check_email_verify&callback=userapi&key=QUgFYq4L5n89ZG2Ab&email=' + this.User + '&verify=' + this.regist.phoneSms).then(function(res) {
			...
			setPass_sub: function() {
				for(let i = 0; i < 1; i++) {
					this.$http.jsonp('http://51kufei.com/index.php?m=Home&c=Userapi&a=changefgPwd&callback=userapi&key=QUgFYq4L5n89ZG2Ab&email='+ this.User +'&new=' + this.regist.password + '&confirm=' + this.regist.con_password).then(function(res) {
