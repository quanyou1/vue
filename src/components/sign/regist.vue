			...
			<div v-show="regfrom" class="login-form">
				<form @submit.prevent="regist" class="regist-form">
					<div class="signUser">
						...
						<input type="text" name="phone_id" id="user_id" placeholder="手机" v-model="regist.phone" />
					</div>
					<div class="verCode">
						<span class="codes">
...
<script>
	...
	export default {
		...
		methods: {
			regist: function() {
				for(let i = 0; i < 1; i++) {
					this.$http.jsonp('http://51kufei.com/index.php?m=Home&c=Userapi&a=sendMsm&callback=userapi&key=QUgFYq4L5n89ZG2Ab&p=' + this.regist.phone + '&verify=4265' + this.regist.verificat).then(function(res) {
						this.data = res.body;
			...
			setPass_sub: function() {
				for(let i = 0; i < 1; i++) {
					this.$http.jsonp('http://51kufei.com/index.php?m=Home&c=Userapi&a=register&callback=userapi&key=QUgFYq4L5n89ZG2Ab&mobile='+ this.User +'&password=' + this.regist.password + '&con_password=' + this.regist.con_password + '&type=1').then(function(res) {
