<template>
	<div class="inforDel-wrapper" :id="id">
		...
		<div class="contents" v-html="contHtml"></div>
		...
		<div class="m_about">
            ...
            <ul>
                <li v-for="relation in relations" @click="getData">
                    <router-link :to="{ path: '/index/details/',query: { id: relation.id }}" replace>
                        {{relation.name}}
                    </router-link>
                </li>
            </ul>
        </div>
		<div id="SOHUCS" :uid="this.routerId"></div>
	</div>
</template>
	  	...
	  	watch: {
	   	...
	   	methods:{
	   		getData() {   //获取数据
                ...
                this.$http.jsonp('http://51kufei.com/index.php?m=Home&c=Userapi&a=home_new_detail&callback=userapi&key=QUgFYq4L5n89ZG2Ab&gid=' + this.routerId).then(function(res){
		           	this.inforItem = res.body.data;
		           	this.relations = res.body.about;
		           	this.contHtml = this.inforItem.content;
		          	console.log(this.inforItem);
		           	this.like= this.inforItem.like;
			        },function(res){
			            console.log(res.status);
		        });
            },
