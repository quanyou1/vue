		...
		<div class="m_about">
            ...
            <ul>
                <li v-for="relation in relations" @click="getData">
                    <router-link :to="{ path: '/index/raceDel/',query: { id: relation.id }}" replace>
                        {{relation.name}}
                    </router-link>
                </li>
            </ul>
	  	...
	  	watch: {
	   	...
	   	methods:{
	    	getData() {   //获取数据
                ...
                this.$http.jsonp('http://51kufei.com/index.php?m=Home&c=Userapi&a=home_competition_detail&callback=userapi&key=QUgFYq4L5n89ZG2Ab&gid=' + this.routerId).then(function(res){
