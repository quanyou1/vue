	...
	<div class="infor-wrapper">	
		...
		<div class="inforContent">
			<div class="infor-list" v-for="(list,id) in inforList">
				<keep-alive>
					<router-link :to="{ path: '/index/raceDel/',query: { id: list.id }}" :id="list.id" :title="list.title">
						<div class="listInfor">
							<h2>{{list.name}}</h2>
							<h3>{{list.summary}}</h3>
...
<script>
	...
	export default {
		...
		created() {
	      ...
	      this.$http.jsonp('http://51kufei.com/index.php?m=Home&c=Userapi&a=home_competition&callback=userapi&key=QUgFYq4L5n89ZG2Ab&p=' + this.p).then(function(res){
